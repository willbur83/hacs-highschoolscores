...
> **Status:** Public **beta** (version names like `0.1.0-beta.5`). Betas are published as GitHub **pre-releases** with the `high_school_sports_scores.zip` asset. Install through HACS (custom repository); you may need to enable or select a pre-release — see [Beta testing](docs/BETA.md). Default HACS **Download** against pre-releases is not guaranteed without following those steps.
...
This project is **not** in the HACS default store. Add the **custom repository** once, then install from the published GitHub Release ZIP (not from the `main` branch). Beta builds are GitHub **pre-releases**; use HACS **Download** / **Redownload** and, if needed, pick the pre-release version per [Beta testing](docs/BETA.md).

1. In Home Assistant, open **HACS** → **Integrations**.
2. Open the menu (⋮) → **Custom repositories**.
3. Add repository **`https://github.com/willbur83/hacs-highschoolscores`**, category **Integration**, then **Add**.
4. Find **High School Sports Scores** in HACS → **Integrations** → **Download** (or **Redownload** when upgrading). Select the current beta pre-release if HACS prompts (for example **`v0.1.0-beta.5`**).
...
| **`Custom element not found: high-school-sports-scores-card`** (especially mobile) | Restart HA after install/upgrade. On **storage-mode** Lovelace (default), the integration registers a **JavaScript module** resource automatically. If you added the same URL manually during testing, remove the duplicate under **Dashboards → Resources** after upgrading. **YAML-mode** Lovelace: add one resource — `type: module`, URL `/high_school_sports_scores/high-school-sports-scores-card.js?v=<your installed version>`. |
